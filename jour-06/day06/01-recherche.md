...
\*_Ma réponse :_
on utiliser la notation pointee si je connais le nom de la propriete , et la notaion crochets si : - on a la propriete dans un variable.
-si la propriete contient des espaces .
-si la propriete n'est pas un identifient javascript valid
...
\*_Ma réponse :_
is on tente d'acceder a une propriete qui n'existe pas dans un objet on a undefined
...
\*_Ma réponse :_
on ajouter une nouvell propriete a un objet deja existant par tape le nom de l'objet dot propriete nom :
exemple : on const person = {name:"---"} pour ajouter le age : person.age = 22
...
\*_Ma réponse :_
pour supprimer un propriete d'un objet on utiliser le mot-cle : delete.
exemple :
const utilisateur = {
nome: "mekkaoui",
age: 26,
...
> Quelle est la boucle spécifique recommandée pour parcourir les _clés_ d'un objet ? (`for...of` ou `for...in`) ?

- [ ] J'ai recherché et compris la notion.

**Ma réponse :*
la boucle recommandee pour parcoourir les cles d'un objet est for...in
*
...
**Ma réponse :*
le mot-cle 'this' désigne pour l'objet
*
